...
CREATE OR ALTER FUNCTION [elasticauditreport].[get_OverviewDBRoleMembers] (
...
FROM [elasticaudit].[T_DBRoleMembers] pr1
INNER JOIN [elasticauditreport].[get_OverviewDatabases_online] (@serverName) AS DBO 
...
LEFT JOIN [elasticauditreport].[get_OverviewDBPrincipals] (@serverName, @dbName) dPrg
  ON dPrg.dtSnapshot=pr1.dtSnapshot AND dPrg.idDB=pr1.idDB AND dPrg.principal_id=pr1.role_principal_id
LEFT JOIN [elasticauditreport].[get_OverviewDBPrincipals] (@serverName, @dbName) dPr
