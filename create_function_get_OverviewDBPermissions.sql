...
CREATE OR ALTER FUNCTION [elasticauditreport].[get_OverviewDBPermissions] (
...
SELECT  pr1.dtSnapshot, pr1.idDB, DBO.Datenbankname,
		...
		pr1.major_id, AllO.name AS major_name, AllO.type AS major_type, AllO.type_desc AS major_type_desc, 
...
FROM [elasticaudit].[T_DBPermissions] pr1
CROSS APPLY (select tbl_dbs.Datenbankname, tbl_dbs.IDSQLServerInstance,  tbl_dbs.strFullInstanceName, tbl_dbs.[strInstanceName], tbl_dbs.[strSQLServerName],
			...
			from [elasticauditreport].[get_OverviewDatabases_online] (@serverName) AS tbl_dbs 
...
CROSS APPLY (select dPrg.name, dPrg.type, dPrg.type_desc
             from [elasticauditreport].[get_OverviewDBPrincipals] (@serverName, @dbName) dPrg
...
OUTER APPLY (select dPr.name, dPr.type, dPr.type_desc
             from [elasticauditreport].[get_OverviewDBPrincipals] (@serverName, @dbName) dPr
			 where dPr.dtSnapshot=pr1.dtSnapshot AND dPr.idDB=pr1.idDB AND dPr.principal_id=pr1.grantor_principal_id
			 ) dPr
OUTER APPLY (select  AllO.name, AllO.type, AllO.type_desc
			 from [elasticauditreport].[get_OverviewDBObjects] (@serverName, @dbName) AllO
...
OUTER APPLY (select rm.member_principal_id, rm.MemberName, rm.MemberType, rm.MemberType_desc,
			 ...
			 from [elasticauditreport].[get_OverviewDBRoleMembers] (@serverName,@dbName) rm			 
